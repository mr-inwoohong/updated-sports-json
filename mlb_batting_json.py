...
def mlb_batting_stats(url):
    ...
    for row in data_rows:
        ...
        data_dict[key] = [{'Year': cols[0], 'Age': cols[1], 'Team': cols[2], 'League': cols[3], 'Games Played': cols[4], 'Plate Appearance': cols[5], 'At Bat': cols[6], ' Runs Scored': cols[7], 'Hits': cols[8], 'Doubles': cols[9], 'Triples': cols[10], 'Homeruns': cols[11], 'RBI': cols[12], 'Stolen Bases': cols[13], 'Caught Stealing': cols[14], 'BB/Walks': cols[15], 'Strikeouts': cols[16], 'Hits/At Bats': cols[17], 'OBP': cols[18], 'SLG': cols[19], 'OPS': cols[20]}]
